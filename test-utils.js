...
function genNewAuction(context, events, done) {
	...
	context.vars.numBids = maxBids;
	context.vars.numQuestions = maxQuestions;
	return done()
}
...
function genNewQuestionR(requestParams, response, context, ee, next) {
	if (response.statusCode == 204) {
		console.log("\n\n\tOwner of auction making question to own auction")
		return next()
	}
	else {
		console.log("\n\n\nRESPOSTA QUESTION REPLY: " + response.body);
		if (response.statusCode >= 200 && response.statusCode < 300 && response.body.length > 0) {
			let a = JSON.parse(response.body)
			questions.push(a)
